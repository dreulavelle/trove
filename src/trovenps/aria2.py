...
def _remote_dir(base: str | None, game: Game, organize: bool) -> str | None:
    """Per-console dir on the aria2 host. Forward slashes — the host may be POSIX."""
    sub = game.platform.folder if (organize and game.platform) else None
    if base and sub:
        return f"{base.rstrip('/')}/{sub}"
    return sub or base  # sub alone is relative to the daemon's default dir


def write_aria2_input(games: list[Game], output_dir: Path, dest: Path, *, organize: bool = True) -> int:
    ...
    for g in targets:
        ...
        lines.append(f"  dir={dest_dir(output_dir, g, organize)}")
        if g.sha256:
...
def run_aria2c(games: list[Game], output_dir: Path, *, concurrency: int = 3, organize: bool = True) -> int:
    ...
    with tempfile.NamedTemporaryFile("w", suffix=".aria2.txt", delete=False, encoding="utf-8") as fh:
        tmp = Path(fh.name)
    try:
        count = write_aria2_input(games, output_dir, tmp, organize=organize)
...
async def add_to_aria2_rpc(
    ...
    *,
    ...
    With ``organize``, each item lands in a ``<console>`` subfolder there.
    """
    targets = [g for g in games if g.downloadable]
    ...
    try:
        for g in targets:
            ...
            if (d := _remote_dir(remote_dir, g, organize)) is not None:
                options["dir"] = d
            if g.sha256:
